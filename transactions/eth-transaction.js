            ...
            body.result.forEach(tx => {
                if (tx.contractAddress === contractAddress.toLowerCase()) {
                    if (tx.to.toLowerCase() === address.toLowerCase()) {
                        ...
                        // check transaction hash with db before making callback and save
                        checkTxAndCallback(tx.tokenSymbol.toLowerCase(), address, tx.from, web3.utils.fromWei(tx.value.toString(), 'ether'), new Date(tx.timeStamp * 1000), tx.hash, tx.blockHash, tx.blockNumber, web3.utils.fromWei(tx.gas.toString(), 'ether'))
                    }
                }
            })
        }
